...
`NONE`, `ERROR` (по умолчанию), `INFO`, `DEBUG`. Уровень `DEBUG` печатает тела запросов и ответов — там бывают персональные данные, в продакшене включать не стоит. Если задан свой `baseUrl`, уровень по умолчанию становится `DEBUG`.
