...
class LiveTest {
    ...
    fun `t16 payload неизменяемой кампании клиент не переписывает`() = runBlocking {
        // Скрипт засева заводит две кампании, различающиеся только этим флагом.
        // Против тенанта, заведённого не им, неизменяемой кампании может не
        // быть — тогда проверка пропускается, а не врёт об успехе.
